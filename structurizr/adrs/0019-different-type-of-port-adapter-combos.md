...
3. Ports whose adapter is expected to be implemented by another context
...
### RE: 3. Ports whose adapter is expected to be implemented by another context
...
>add a third piece in an integration / anti-corruption layer (or just a technical module that’s allowed to depend on both):
...
However, we might test both alternatives in a toy project: The more complicated two ports version AND the message bus architecture.

## Consequences

We will have a lower structural overhead for now and will implement a more complex version if needed in the future, when one provider serverse multiple consumers.
