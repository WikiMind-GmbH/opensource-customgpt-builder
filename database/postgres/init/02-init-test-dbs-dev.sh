...
# Set up database knowledge_context_test_db
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "knowledge_context_test_db" <<'SQL'
  REVOKE CONNECT ON DATABASE knowledge_context_test_db FROM PUBLIC;
  REVOKE ALL ON SCHEMA public FROM PUBLIC;  
  GRANT CONNECT ON DATABASE knowledge_context_test_db TO knowledge_context_test_app;
  
  CREATE SCHEMA knowledge_schema;
  ALTER ROLE knowledge_context_test_app SET search_path = knowledge_schema, pg_catalog;
  GRANT USAGE, CREATE ON SCHEMA knowledge_schema TO knowledge_context_test_app;
SQL
