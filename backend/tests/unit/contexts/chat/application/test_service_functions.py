...
def test_continue_conversation_messages_are_added_to_conversation(
    ...
    with conv_uow_factory() as verification_uow: #explicitly not creating Message objects for slightly better decoupling from domain model
...
def test_continue_conversation_llm_adapter_populated_correctly(
    ...
    with cgpt_uow_factory() as uow:
        cgpt:CustomGPT = uow.cgpt_repo.create_cgpt(name="cgpt_1", instructions="Do sth")
        cgpt_id: str = cgpt.id
        uow.commit()
    ...
    sysprompt_was_passed = len(fake_llm_adapter_cgpt_exists.function_call_parameters[0].cgpt_systemprompt) > 0
