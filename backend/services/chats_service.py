...
async def get_chat_history_helper(chat_id: int) -> ChatHistory:
    ...
    try:
        ...
        chat_history_list = ChatHistory(
            ...
            messages=[
                ...
                SimplifiedMessage(
                    role=Role.assistant, message="Hi, how can I help you?"
                ),
            ],
        )
...
async def get_chat_summaries_helper() -> list[ChatSummary]:
    ...
    try:
        ...
        logger.exception(
            "Unexpected error occurred while retrieving chat summaries list", err
        )
        raise HTTPException(status_code=500, detail="Internal server error")
