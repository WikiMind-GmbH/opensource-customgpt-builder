...
def cgpt_infos_to_sysprompt(
    ...
    msgs.append(
        MessageDTORetreiver(
            role=RoleDTORetreiver.system, text_content=f"You are `{cgpt_name}`)"
        )
    )
    msgs.append(
        MessageDTORetreiver(
    ...
    msgs.append(
        MessageDTORetreiver(
            role=RoleDTORetreiver.system, text_content=cgpt_instructions
        )
    )
    msgs.append(MessageDTORetreiver(role=RoleDTORetreiver.system, text_content="```"))
    msgs.append(
    ...
    def get_cgpt_sys_prompt(
        ...
        try:
            with self._cgpt_uow_factory() as uow:
                ...
                messages_of_sysprompt: list[MessageDTORetreiver] = (
                    cgpt_infos_to_sysprompt(
