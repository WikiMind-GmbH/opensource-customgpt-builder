...
from src.contexts.chat.application.service_functions import continue_conversation, create_conversation, get_chat_summaries_service, retrieve_chat_history_by_id
...
async def conversation_non_existent_exception_handler(
...
# ToDo add exception handlers for all other cutsom Exceptioins of Ports
...
def get_chat_summaries(
    ...
    overviews: List[ConversationOverview] = get_chat_summaries_service(conv_uow)
...
async def get_chat_history(
    ...
    history:ConversationFilteredForClient = retrieve_chat_history_by_id(chat_id, conv_uow)
    history_to_old_schema: list[SimplifiedMessage] = [SimplifiedMessage(role = Role(str(msg.role)), message=msg.text) for msg in history.messages]
    return ChatHistory(custom_gpt_id=history.customgpt_id, messages=history_to_old_schema)
#------------------------------------------------------------------------ 
# WiP
@app.post(
    "/send-user-message",
...
async def send_user_message(
    ...
    cgpt_retreiver: CustomGPTInstructionsRetreiver = Depends(deps.cgpt_instructions_adapter_factory)
) -> AssistantMessage:
    if isinstance(request, NewChatRequest):
        conversation_id: str = create_conversation(custom_gpt_id=request.custom_gpt_id, conv_uow=conv_uow)
    else:
        conversation_id: str = request.conversation_id
    response: str = continue_conversation(conv_id=conversation_id, conv_uow=conv_uow, cgpt_retreiver=cgpt_retreiver,llm_adapter=llm_adapter)
    return AssistantMessage(conversation_id=conversation_id, response_message=SimplifiedMessage(role= Role.assistant, message=response))
